...
In this document, I describe how the GEO data entry for the Camcap data was processed and saved into an object for analysis in Bioconductor. First of all load the relevant libraries for grabbing and manipulaing the data
...
pd2 <- data.frame("geo_accession" = pd$geo_accession, Sample = pd$description, Sample_Group = Group,Gleason=gsub("tumour gleason: ","",pd$characteristics_ch1.1), 
                            ...
                            TotalTime = gsub("time to bcr (months): ","",pd$characteristics_ch1.7,fixed=TRUE),ERG = gsub("tmprss2: ERG gene fusion status: ","",pd$characteristics_ch1.8,fixed=TRUE),
...
iqr <- apply(eMat, 1, IQR,na.rm=TRUE)
pq <- unlist(mget(rownames(eMat),illuminaHumanv4PROBEQUALITY))

minGeneInfo <- data.frame(fData(camcap)[,c("Symbol","Entrez_Gene_ID")],iqr,ProbeQuality=pq)



write.csv(data.frame(minGeneInfo,eMat), file="camcapPerSampleExpression.csv",quote=FALSE)

colnames(eMat) <- pData(camcap)$Sample
library(limma)

des <- model.matrix(~0+as.factor(pData(camcap)$Sample_Group))
colnames(des) <- c("Benign","CRPC","Tumour")
aveVals <- lmFit(eMat,des)

write.csv(data.frame(minGeneInfo,aveVals$coeff),file="camcapPerSampleTypeExpression.csv",quote=FALSE)

haveGrp <- !is.na(pData(camcap)$iCluster)

des <- model.matrix(~0+as.factor(pData(camcap)$iCluster[haveGrp]))
colnames(des) <- paste("iCluser",1:5)
aveVals <- lmFit(eMat[,haveGrp],des)

write.csv(data.frame(minGeneInfo,aveVals$coeff),file="camcapPeriClusterExpression.csv",quote=FALSE)
