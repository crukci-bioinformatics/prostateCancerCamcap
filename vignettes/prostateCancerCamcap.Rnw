...
weirdValue <- setdiff(pd2$iCluster, c("clust1","clust2","clust3","clust4","clust5",NA))

if(length(weirdValue) > 0) pd2$iCluster[pd2$iCluster %in% weirdValue] <- NA

pd2$Gleason <- gsub("N/A", NA, pd2$Gleason)
weirdValue <- setdiff(pd2$Gleason, c("10=5+5","6=3+3","7=3+4","7=4+3","8=3+5","9=5+4",NA))

if(length(weirdValue) > 0) pd2$Gleason[pd2$Gleason %in% weirdValue] <- NA
pd2$Gleason <- factor(pd2$Gleason, levels = c("6=3+3","7=3+4","7=4+3","8=3+5","9=5+4","10=5+5"))
